...
public class BaseMorphScreen extends Screen {
    ...
    public boolean mouseScrolled(double pMouseX, double pMouseY, double scrollX, double scrollY) {
        ...
        if (!superMs) {
            if (pMouseX > otherListsMinX && pMouseX < otherListsMaxX + 4
                ...
                currentOtherListYPos -= (int) (scrollY * scrollSpeed);
    ...
    public void renderCat(GuiGraphics pGuiGraphics, int pX, int pY, int pScale, float pMouseX, float pMouseY, LivingEntity pEntity) {
        ...
        InventoryScreen.renderEntityInInventory(pGuiGraphics, pX, pY, pScale, new Vector3f(0,0,0), quaternionf, quaternionf1, pEntity);
        pEntity.yBodyRot = f2;
        pEntity.setYRot(f3);
        pEntity.setXRot(f4);
